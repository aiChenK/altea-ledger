...
- ⚡ **智能周期重置**：
  - **日常重置**：每日默认 **09:00** 自动重置日常状态（签到、喝药、幸运、挂机等），并支持特定日常任务设置独立的重置时间（如“挂机”可单独设定为 **00:00** 重置）。
...
- 📧 **邮件自动与手动备份**：
  - 支持绑定发信邮箱（如 QQ、163 邮箱）。在每周重置清除周常进度前，自动在后台静默将数据作为附件发送至指定邮箱。
  - 支持在前端设置抽屉中一键“发送测试备份邮件”，方便用户即时备份。
...
| `PORT` | 服务端口 | `3001` |
| `SMTP_HOST` | 邮件服务器 SMTP 主机名 | `smtp.qq.com` |
| `SMTP_PORT` | 邮件服务器 SMTP 端口 | `465` (使用 SSL) |
| `SMTP_USER` | 用于发信的邮箱账号 | `sender_account@qq.com` |
| `SMTP_PASS` | 用于发信的邮箱 SMTP 授权密码 (非邮箱登录密码) | `abcdefghijklmnop` |
